...
void drawPipe(u_char ballX, u_char ballY) {
  /* compute:     ball left & right
     rectangular window that includes old & new ball */
  u_char ballTop = ballY - pipeRadius, ballBot = ballY + pipeRadius;
  u_char ballTopMajor = getYMajor(ballTop), ballBotMajor = getYMajor(ballBot);
  u_char ballLeft = ballX - pipeRadius, ballRight = ballX + pipeRadius;
