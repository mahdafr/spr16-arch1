...
u_char getChunk(u_char delY, u_char rowMajor) {
  ...
  u_char dotYMajor = lcd_getYMajor(dotY), dotYMinor = lcd_getYMinor(dotY);
  if ( dotYMajor == rowMajor )
    deChunk += chunkMasks[dotYMinor] ^ chunkMasks[dotYMinor+1]; //draw at dotY
  return deChunk;
}
u_char loopDeLoop(u_char start, u_char stop, u_char rowMajor) {
  u_char deChunk, delY;
  for ( delY=start ; delY<stop ; delY++ )
    deChunk += getChunk(delY,rowMajor);
  return deChunk;
}
