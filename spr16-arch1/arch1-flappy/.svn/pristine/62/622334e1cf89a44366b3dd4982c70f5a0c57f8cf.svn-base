...
short pipePos[6], pipeGapTop[6], pipeGapHeight[6];
short pipe = 0, pipeXPos = 6, pipeGapTop = MAX_Y/2, pipeGapHeight = 15, pipeVel = -1;

void drawPipe(u_char pipeX, u_char gapHeight, u_char gapTop) {
  pipePos[pipe] = pipeX, pipeGapHeight[pipe] = gapHeight, pipeGapTop[pipe] = gapTop;
  ...
  if ( pipe>=5 ) {
    ...
    for ( ; rowMajor<=rowMajorBound, rowMajor2<=rowMajorBound2 ;	    // for all affected chunk #s
      ...
      u_char chunk = 0, col;
      //TODO: does this delete the random chunks draw on the left as the other side of the pipe?
      if ( (pipeLeft>=MAX_X && pipeRight<=0) || (pipeRight<=pipeLeft) )
	continue;
