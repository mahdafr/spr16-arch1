...
void updatePipe(u_char numColsToMoveLeft) {
  ...
  for ( col=0 ; col<MAX_X ; col++ ) {
	...
	for ( rowMajor=0 ; rowMajor<9 ; rowMajor++ ) {
      ...
      col = prevPipePos + 3*pipeRadius; //skip to next pipe's start
      pipe++;
      if ( pipe>=5 ) pipe = 0;
    } else {
  ...
  /*moveLeft(0,numColsToMoveLeft);
  moveLeft(1,numColsToMoveLeft);
  moveLeft(2,numColsToMoveLeft);
  moveLeft(3,numColsToMoveLeft);
  moveLeft(4,numColsToMoveLeft);*/
}
